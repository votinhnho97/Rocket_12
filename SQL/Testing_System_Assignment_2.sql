...
SELECT d.DepartmentID, d.DepartmentName, p.PositionID, p.PositionName, count(*) AS Num_of_Staff
FROM `Account` a
INNER JOIN `Position` p ON a.PositionID = p.PositionID
INNER JOIN `Department` d ON a.DepartmentID = d.DepartmentID
...
SELECT q.QuestionID, q.Content, a.Fullname, q.CreateDate, cq.CategoryName, tq.TypeName
FROM `Question` q
